...
export const handler: lambda.ValidatedAPIGatewayProxyEventHandler<Schema> = lambda.middify(async (event) => {
  const {
...
const transactionPut = (accountId: string, amount: number, denomination: string, credit = false) => {
